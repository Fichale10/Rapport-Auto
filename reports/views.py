...
DR2_DASHBOARD_REGION_ORDER = ['SAVANES', 'KARA', 'CENTRALE', 'PLATEAUX', 'MARITIME', 'LOME']

# Centres approximatifs des régions (lat, lon) — repli pour les sites sans
# coordonnées GPS renseignées dans la table Site (mêmes valeurs que home.html).
DR2_DASHBOARD_REGION_CENTERS = {
    'SAVANES':  (10.5118, 0.432),
    'KARA':     (9.5402, 0.9226),
    'CENTRALE': (8.6036, 1.0897),
    'PLATEAUX': (7.4651, 1.1382),
    'MARITIME': (6.5174, 1.2655),
    'LOME':     (6.1631, 1.2869),
}


def _dr2_site_severity(count):
    if count >= 4:
        return 'CRITIQUE', 'red'
    if count >= 2:
        return 'MOYEN', 'orange'
...
def _dr2_jitter(site_name, region):
    """Décalage déterministe (basé sur le nom du site) autour du centre de
    région, utilisé quand le site n'a pas de coordonnées GPS renseignées."""
    import hashlib
    h = int(hashlib.md5(site_name.encode('utf-8')).hexdigest(), 16)
    dlat = ((h % 1000) / 1000 - 0.5) * 0.3
    dlon = (((h // 1000) % 1000) / 1000 - 0.5) * 0.3
    base_lat, base_lon = DR2_DASHBOARD_REGION_CENTERS.get(region, (8.6, 1.0))
    return base_lat + dlat, base_lon + dlon


def dr2_dashboard_view(request):
    """Vue graphique DR2 : gauges par région, carte du Togo avec les sites en
    violation DR2, courbe d'évolution quotidienne et liste recherchable des
    sites — calculé depuis `Dr2ViolationRecord` (données 2G/3G automatiques)."""
    from .models import Dr2ViolationRecord, Site

    today = date.today()
    mois_str = request.GET.get('mois', '').strip()
    try:
        y_str, m_str = mois_str.split('-')
        mois_debut = date(int(y_str), int(m_str), 1)
    except (ValueError, TypeError):
        mois_debut = date(today.year, today.month, 1)
    last_day  = calendar.monthrange(mois_debut.year, mois_debut.month)[1]
    mois_fin  = date(mois_debut.year, mois_debut.month, last_day)

    if mois_debut.month == 1:
        prev_mois = date(mois_debut.year - 1, 12, 1)
    else:
        prev_mois = date(mois_debut.year, mois_debut.month - 1, 1)
    if mois_debut.month == 12:
        next_mois = date(mois_debut.year + 1, 1, 1)
    else:
        next_mois = date(mois_debut.year, mois_debut.month + 1, 1)

    records = list(
        Dr2ViolationRecord.objects
        .filter(date__gte=mois_debut, date__lte=mois_fin)
        .values('region', 'site_name', 'date')
    )

    # Certains enregistrements DR2 ont un champ `region` vide (site absent au
    # moment du calcul automatique) → on complète via la table Site quand possible.
    blank_names = {rec['site_name'] for rec in records if not (rec['region'] or '').strip()}
    site_region_fallback = {
        s['site_name']: (s['region'] or '').strip().upper()
        for s in Site.objects.filter(site_name__in=blank_names).values('site_name', 'region')
    }

    region_counts = {r: 0 for r in DR2_DASHBOARD_REGION_ORDER}
    region_sites  = {r: set() for r in DR2_DASHBOARD_REGION_ORDER}
    site_counts   = {}
    site_region   = {}
    daily_counts  = {}
    for rec in records:
        region = (rec['region'] or '').strip().upper() or site_region_fallback.get(rec['site_name'], '') or 'INCONNU'
        region_counts[region] = region_counts.get(region, 0) + 1
        region_sites.setdefault(region, set()).add(rec['site_name'])
        site_counts[rec['site_name']] = site_counts.get(rec['site_name'], 0) + 1
        site_region[rec['site_name']] = region
        daily_counts[rec['date']] = daily_counts.get(rec['date'], 0) + 1

    total_dr2 = len(records)
    max_region_count = max(region_counts.values()) if region_counts else 0

    region_rows = []
    for region in DR2_DASHBOARD_REGION_ORDER:
        cnt = region_counts.get(region, 0)
        pct = round(cnt / max_region_count * 100) if max_region_count else 0
        if cnt == 0:
            color = 'none'
        elif cnt == max_region_count:
            color = 'red'
        elif pct >= 40:
            color = 'yellow'
        else:
        ...
        region_rows.append({
            'region': region,
            'count':  cnt,
            'sites':  len(region_sites.get(region, ())),
            'pct':    pct,
            'dash':   round(157.1 * pct / 100, 1),
            'color':  color,
        })

    site_rows = []
    for name, cnt in site_counts.items():
        label, color = _dr2_site_severity(cnt)
        site_rows.append({'site_name': name, 'count': cnt, 'label': label, 'color': color})
    site_rows.sort(key=lambda r: (-r['count'], r['site_name']))

    site_geo = {
        s['site_name']: s
        for s in Site.objects.filter(site_name__in=site_counts.keys())
                              .values('site_name', 'latitude', 'longitude')
    }
    sites_geo_list = []
    for row in site_rows:
        name   = row['site_name']
        region = site_region.get(name, '')
        rec    = site_geo.get(name)
        if rec and rec['latitude'] is not None and rec['longitude'] is not None:
            lat, lon, approx = rec['latitude'], rec['longitude'], False
        else:
            lat, lon = _dr2_jitter(name, region)
            approx = True
        sites_geo_list.append({
            'name': name, 'lat': lat, 'lon': lon, 'count': row['count'],
            'label': row['label'], 'color': row['color'], 'region': region, 'approx': approx,
        })

    # ── Courbe d'évolution quotidienne (mois en cours → jusqu'à aujourd'hui) ──
    end_for_trend = min(mois_fin, today) if (mois_debut.year, mois_debut.month) == (today.year, today.month) else mois_fin
    trend = []
    d = mois_debut
    while d <= end_for_trend:
        trend.append({'label': d.strftime('%d/%m'), 'count': daily_counts.get(d, 0)})
        d += timedelta(days=1)

    ctx = {
        'mois_value':  mois_debut.strftime('%Y-%m'),
        'mois_label':  f'{MOIS_FR_LONG[mois_debut.month].upper()} {mois_debut.year}',
        'prev_mois':   prev_mois.strftime('%Y-%m'),
        'next_mois':   next_mois.strftime('%Y-%m'),
        'total_dr2':   total_dr2,
        'region_rows': region_rows,
        'site_rows':   site_rows,
        'sites_geo_json': json.dumps(sites_geo_list),
        'trend_json':     json.dumps(trend),
    }
    return render(request, 'reports/dr2_dashboard.html', ctx)
